        ...
        private async Task LoadData()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        await dataGridView1.InvokeAsync(() =>
                        dataGridView1.Rows.Add([item.Scan_ID,item.Scan_ID2, item.CurrentStatus, item.StartTime, item.ScrewStartTime, item.ScrewEndTime, item.LaserStartTime, item.LaserEndTime, item.CameraStartTime, item.CameraEndTime,item.CHECKSUM])
                        );
                    }
